import { CAMERA_HEIGHT, JUMP_AIRTIME_MS, LANE_TWEEN_MS, LANES, MAX_SPEED, PLAYER_ACCEL, ROAD_WIDTH } from '../config';
...
export class Player {
  ...
  /** Space/Up: fixed-impulse jump with constant airtime (§4.3). No double-jump. */
  requestJump(): void {
    if (this.airborne) {
      return;
    }
   ...
   *  into world-Y/camera math. */
  get jumpArcHeight(): number {
    if (!this.airborne) {
      return 0;
    }
    const t = this.jumpElapsedMs / JUMP_AIRTIME_MS;
    return 4 * t * (1 - t);
  }
  ...
  private updateJump(deltaMs: number): void {
    ...
    if (this.jumpElapsedMs >= JUMP_AIRTIME_MS) {
      this.airborne = false;
      this.jumpElapsedMs = 0;
    }
  }
